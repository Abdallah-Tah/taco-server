...
export async function setupAxiosProxy() {
    ...
    try {
        ...
        const isSocks = config.proxyUrl.startsWith('socks');
        if (isSocks) {
            const { SocksProxyAgent } = await import('socks-proxy-agent');
            axiosAgent = new SocksProxyAgent(config.proxyUrl);
            fetchDispatcher = new SocksProxyAgent(config.proxyUrl); // reuse for fetch
            logger.info(`Axios proxy configured → ${maskProxyUrl(config.proxyUrl)}`);
            logger.info(`Fetch proxy configured (SOCKS) → ${maskProxyUrl(config.proxyUrl)}`);
        } else {
    ...
    if (!fetchDispatcher) {
        try {
            // 2. Setup undici ProxyAgent (for native fetch, HTTP/HTTPS only)
...
export async function proxyFetch(url, opts = {}) {
    if (fetchDispatcher) {
        // For SOCKS: use the agent via http/https module
        const isSocks = config.proxyUrl?.startsWith('socks');
        if (isSocks) {
            const parsed = new URL(url);
            const httpModule = parsed.protocol === 'https:' ? await import('https') : await import('http');
            return new Promise((resolve, reject) => {
                const req = httpModule.default.get(url, { agent: fetchDispatcher, ...opts }, (res) => {
                    let body = '';
                    res.on('data', (chunk) => body += chunk);
                    res.on('end', () => {
                        resolve({ ok: res.statusCode >= 200 && res.statusCode < 300, status: res.statusCode, json: () => JSON.parse(body), text: () => body, headers: res.headers });
                    });
                });
                req.on('error', reject);
                if (opts.signal) opts.signal.addEventListener('abort', () => req.destroy());
            });
        }
...
async function checkOutboundIP() {
    ...
    if (fetchDispatcher) {
        const isSocks = config.proxyUrl?.startsWith('socks');
        try {
            const resp = await proxyFetch(IP_SERVICE, { signal: AbortSignal.timeout(10000) });
            const proxyData = await resp.json();
            logger.info(`Proxy IP      : ${proxyData.ip} ← Polymarket sees this`);
        } catch {
        ...
        try {
            const resp = await proxyFetch(GEOBLOCK_API, { signal: AbortSignal.timeout(10000) });
...
export async function testProxy() {
    ...
    try {
        ...
        const resp = await proxyFetch(`${config.clobHost}/time`, {

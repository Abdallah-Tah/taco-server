...
/**
 * Direct EOA call — when proxyWallet is NOT a Gnosis Safe (plain EOA).
 */
async function _doDirectCall(to, data, description = '', gasLimit = undefined) {
    ...
    const tx = await wallet.sendTransaction({ to, data, ...txOpts });
    logger.info(`MM: direct tx sent ${tx.hash}`);
...
async function _doExecSafeCall(to, data, description = '', gasLimit = undefined) {
    ...
    // Check if proxyWallet is a contract (Gnosis Safe) or EOA
    const provider = await getPolygonProvider();
    const code = await provider.getCode(config.proxyWallet);
    const isSafe = code !== '0x';

    if (!isSafe) {
        // Direct EOA execution — wallet is the signer itself
        return _doDirectCall(to, data, description, gasLimit);
    }

    // ... Safe execution path below ...

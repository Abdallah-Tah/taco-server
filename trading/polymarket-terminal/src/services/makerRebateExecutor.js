...
async function monitorSimFill(pos, tag, label) {
    const fillDelayMs = 10_000 + Math.random() * 30_000; // Fill in 10–40s
    const bothFillChance = 0.55; // 55% chance both sides fill
    const oneFillChance = 0.25;  // 25% chance only one side
    // 20% chance neither fills

    logger.info(`MakerMM${tag}: [SIM] monitoring — will check fill in ${(fillDelayMs / 1000).toFixed(0)}s`);
    await sleep(fillDelayMs);

    const roll = Math.random();

    if (roll < bothFillChance) {
        // Both sides filled — simulate merge profit
        const shares = pos.targetShares;
        const totalCost = pos.yes.cost + pos.no.cost;
        const recovered = shares;
        pos.totalProfit = recovered - totalCost;
        pos.yes.filled = true;
        pos.no.filled = true;
        logger.money(`MakerMM${tag}: [SIM] YES filled ${shares} shares`);
        logger.money(`MakerMM${tag}: [SIM] NO filled ${shares} shares`);
        // Single summary notification only
        pos.status = 'done';
    } else if (roll < bothFillChance + oneFillChance) {
        // Only one side filled — simulate cut-loss
        const side = Math.random() < 0.5 ? 'yes' : 'no';
        const other = side === 'yes' ? 'no' : 'yes';
        pos[side].filled = true;
        pos[other].filled = false;
        const cost = pos[side].cost;
        pos.totalProfit = -cost;
        logger.warn(`MakerMM${tag}: [SIM] only ${side.toUpperCase()} filled — cut-loss`);
        // Cut-loss handled in summary below
        pos.status = 'done';
    } else {
        // Neither filled — timeout
        pos.totalProfit = 0;
        logger.info(`MakerMM${tag}: [SIM] neither side filled — timeout`);
        // Timeout handled in summary below
        pos.status = 'done';
    }

    const sign = pos.totalProfit >= 0 ? '+' : '';
    const emoji = pos.totalProfit >= 0 ? '🟢' : '🔴';
    logger.info(`MakerMM${tag}: [SIM] done | P&L: ${sign}$${pos.totalProfit.toFixed(2)}`);
    notify(`${emoji}🌮 [SIM] ${tag} | P&L: ${sign}$${pos.totalProfit.toFixed(2)} | YES $${pos.yes.buyPrice} + NO $${pos.no.buyPrice} = $${(pos.yes.buyPrice + pos.no.buyPrice).toFixed(4)}`);
}

async function monitorUntilFilled(pos, tag, label) {
    // ── SIM MODE: simulate fills without on-chain checks ─────────────────
    if (config.dryRun) {
        return monitorSimFill(pos, tag, label);
    }
    ...
    try {
        ...
        while (true) {
            ...
            // Both sides have net balance ≥ 50% target → mark done, let resolution handle it
            if (yesShares >= pos.targetShares * 0.5 && noShares >= pos.targetShares * 0.5) {
                pos.bothFilledSince = null;
                ...
                logger.success(
                    ...
                    `YES=${yesShares.toFixed(4)} NO=${noShares.toFixed(4)}. Holding for resolution.`
                );
                pos.yes.filled = true;
                pos.no.filled = true;
            ...
            if (msRemaining <= config.makerMmCutLossTime * 1000) {
                ...
                if (yesShares >= 1 && noShares >= 1) {
                    // Both sides have net fills — no merge needed, let resolution handle it
                    const minShares = Math.min(yesShares, noShares);
                    logger.info(`MakerMM${tag}: both sides filled at cut-loss — holding for resolution (${minShares.toFixed(4)} pairs)`);
                    pos.totalProfit = minShares - (pos.yes.cost + pos.no.cost);
                    pos.status = 'done';
                    pos.merged = false;
                } else {
                    // One or neither side net-filled — cancel open orders, try CLOB sell
                    await Promise.all([
                        cancelOrder(pos.yes.orderId),
                        cancelOrder(pos.no.orderId),
                    ]);
                    if (yesShares > 0 || noShares > 0) {
                        // IMPROVEMENT: Try to sell the filled side on CLOB to minimize loss
                        // instead of holding directional exposure until resolution
                        const filledSide = yesShares > 0 ? 'yes' : 'no';
                        const shares = Math.max(yesShares, noShares);
                        const tokenId = pos[filledSide].tokenId;
                        logger.info(`MakerMM${tag}: attempting CLOB sell for ${shares.toFixed(1)} ${filledSide.toUpperCase()} shares`);
                        try {
                            const sellPrice = await getBestBid(tokenId);
                            if (sellPrice && sellPrice > 0) {
                                logger.info(`MakerMM${tag}: selling ${filledSide.toUpperCase()} @ $${sellPrice} × ${shares.toFixed(1)}`);
                                // Sell via market order (FOK) to exit quickly
                                const sellResult = await clobClient.postOrder(
                                ...
                                logger.info(`MakerMM${tag}: sell order result: ${JSON.stringify(sellResult)}`);
                                pos.totalProfit = (shares * sellPrice) - pos[filledSide].cost;
                            } else {
                                logger.warn(`MakerMM${tag}: no CLOB bid — holding for resolution`);
                                pos.totalProfit = -(pos[filledSide].cost);
                            }
                        } catch (sellErr) {
                            logger.warn(`MakerMM${tag}: CLOB sell failed: ${sellErr.message} — holding for resolution`);
                            pos.totalProfit = -(pos[filledSide].cost);
                        }
                        pos.oneSided = true;
                    } else {
...
async function executeMerge(pos, shares, tag) {
    ...
    // IMPROVEMENT: Skip merge, let resolution handle it.
    // Merge requires USDC.approve() which is broken for our EOA on Polygon.
    // Resolution + auto-redeem achieves the same result — just takes longer.
    // We already proved this works in the live test ($0.40 profit).
    logger.info(`MakerMM${tag}: skipping merge — letting positions resolve naturally (both sides filled)`);
    logger.money(`MakerMM${tag}: estimated P&L +$${pos.totalProfit.toFixed(2)} (will confirm on redeem)`);
    pos.status = 'done';
    pos.merged = false; // flag: needs auto-redeem to confirm P&L
}
...
export async function executeMakerRebateStrategy(market) {
    ...
    while (true) {
        ...
        const edge = (1 - combined).toFixed(4);
        notify(`📍🌮 [MAKER] ${tag} entering | YES $${yesBid} + NO $${noBid} = $${combined.toFixed(4)} | edge $${edge}`);
        break;
    }
    ...
    const emoji = pos.totalProfit >= 0 ? '🟢' : '🔴';
    notify(`${emoji}🌮 [MAKER] ${tag} | P&L: ${sign}$${pos.totalProfit.toFixed(2)} | YES $${pos.yes.buyPrice} + NO $${pos.no.buyPrice} = $${(pos.yes.buyPrice + pos.no.buyPrice).toFixed(4)}`);

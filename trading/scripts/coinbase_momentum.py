...
CB_GRID_SPACING = env_float('CB_GRID_SPACING', 200)  # Default for BTC
...
CB_BTC_SPACING = env_float('CB_BTC_SPACING', CB_GRID_SPACING)
CB_ETH_SPACING = env_float('CB_ETH_SPACING', 20)  # ETH needs tighter spacing
CB_BTC_PROFIT = env_float('CB_BTC_PROFIT', CB_GRID_PROFIT)
CB_ETH_PROFIT = env_float('CB_ETH_PROFIT', 30)  # ETH needs smaller profit target
...
class Bot:
    ...
    def ensure_grid(self, pair: str, current_price: float):
        ...
        if need_reset and not active_holding:
            ...
            # Use pair-specific spacing and profit
            spacing = CB_BTC_SPACING if 'BTC' in pair else CB_ETH_SPACING
            profit = CB_BTC_PROFIT if 'BTC' in pair else CB_ETH_PROFIT
            new_levels = []
            ...
            log.info('[CB-GRID] Reset grid for %s around $%.2f | spacing=%s profit=%s levels=%s', pair, current_price, spacing, profit, [(l.buy_price, l.sell_price) for l in new_levels])
            self._save_state()

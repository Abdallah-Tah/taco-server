    ...
    if POLY_POSITIONS.exists():
        try:
            ...
            if isinstance(positions, dict):
                ...
                items = [(pos.get("token_id"), pos) for pos in positions if pos.get("token_id")]
            else:
                items = []
            for token_id, pos in items:
                try:
                    r = tor.get(
...
def snapshot() -> dict:
    ...
    # Check milestones
    newly_hit = check_milestones(total_usd, portfolio.get("reached_milestones", []))
    if newly_hit:
        for m in newly_hit:
...
def main():
    ...
    get_db()  # Initialize DB connection

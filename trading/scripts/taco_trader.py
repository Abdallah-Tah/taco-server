...
from config import (
    ...
    SECOND_TP_FRACTION, MIN_RESERVE_SOL, STOP_TRADING_SOL,
...
WHALE_FILE     = Path("/home/abdaltm86/.openclaw/workspace/trading/whale_wallets.json")
...
def load_whale_wallets() -> list:
    if WHALE_FILE.exists():
        try:
            ...
            log(f"Whale file error: {e}")
    return []


def is_graduated_pump_pair(pair: dict, mint: str) -> bool:
    dex_id = str(pair.get("dexId", "") or "").lower()
    labels = " ".join(pair.get("labels") or []).lower()
    url = str(pair.get("url", "") or "").lower()
    return dex_id == "raydium" and (mint.lower().endswith("pump") or "pump" in labels or "pump" in url)


def recent_buys_for_wallet(wallet: str, limit: int = 8) -> list:
    rpc = "https://api.mainnet-beta.solana.com"
    out = []
    try:
        res = requests.post(
            rpc,
            json={"jsonrpc": "2.0", "id": 1, "method": "getSignaturesForAddress", "params": [wallet, {"limit": limit}]},
            timeout=12,
        ).json().get("result", [])
        for it in res:
            ...
            tx = requests.post(
                rpc,
                json={"jsonrpc": "2.0", "id": 1, "method": "getTransaction", "params": [sig, {"encoding": "jsonParsed", "maxSupportedTransactionVersion": 0}]},
                timeout=15,
            ).json().get("result")
            if not tx:
                continue
            meta = tx.get("meta", {})
            pre = {
                b.get("mint"): float((b.get("uiTokenAmount") or {}).get("uiAmount") or 0)
                for b in meta.get("preTokenBalances", []) if b.get("owner") == wallet and b.get("mint")
            }
            post = {
                b.get("mint"): float((b.get("uiTokenAmount") or {}).get("uiAmount") or 0)
                for b in meta.get("postTokenBalances", []) if b.get("owner") == wallet and b.get("mint")
            }
        ...
        log(f"Whale scan error {wallet[:6]}: {e}")
    return out


def whale_candidate_mints(existing_mints: set) -> set:
    whales = load_whale_wallets()[:5]
    mints = set()
    for wallet in whales:
        for mint in recent_buys_for_wallet(wallet):
...
def scan_opportunities(existing_mints: set) -> list:
    """Copy-trade mode: only evaluate recent whale buys that look like graduated pump.fun tokens on Raydium."""
    candidates = []
    followed_mints = whale_candidate_mints(existing_mints)
    log(f"Whale-follow: {len(followed_mints)} recent unique mints from watch wallets")
    for addr in followed_mints:
        ...
        try:
            ...
            if not pairs:
                continue
            p = pairs[0]
            if not is_graduated_pump_pair(p, addr):
                continue
            ...
            candidates.append({
                ...
                "source":      "whale_follow+graduated_pump",
            })
        except Exception as e:
            log(f"Candidate scan error {addr[:6]}: {e}")
...
def buy_size_for_candidate(c: dict, sol_bal: float, regime: dict) -> float:
    """Return SOL size while respecting gas reserve and whale-follow sizing cap."""
    high_conv = (
    ...
    free_sol = max(0.0, round(sol_bal - MIN_RESERVE_SOL, 3))
    return min(size, free_sol)
...
while True:
    ...
    for mint, pos in list(positions.items()):
        try:
            ...
            if exit_type == "PARTIAL_TP":
                ...
                out = execute_sell(mint, SECOND_TP_FRACTION)
                log(f"  Sell result: {out[:150]}")
                if "Signature" in out:
                    pos["tp_hit"]     = True
                    pos["entry"]      = price
    ...
    if sol_bal <= STOP_TRADING_SOL:
        log(f"SOL at/below stop threshold ({sol_bal:.4f} <= {STOP_TRADING_SOL:.4f}) — waiting")
    elif open_slots > 0 and sol_bal >= (DEFAULT_RISK_SOL + MIN_RESERVE_SOL):
        ...
        for qc in list(_signal_queue):
            ...
            if buy_sol < DEFAULT_RISK_SOL or (sol_bal - buy_sol) < MIN_RESERVE_SOL:
                continue
        ...
        elif sol_bal < (DEFAULT_RISK_SOL + MIN_RESERVE_SOL):
            log(f"Insufficient free SOL ({sol_bal:.4f}) with reserve={MIN_RESERVE_SOL:.3f} — waiting")

...
New env vars:
  SOL15M_DISCOUNT_MAKER_EXEC (bool, default false) — enable real maker execution
  SOL15M_DISCOUNT_FILL_TIMEOUT_SEC (int, default 120) — seconds to wait for fills
  SOL15M_DISCOUNT_FILL_POLL_SEC (int, default 4) — poll interval while waiting
  SOL15M_DISCOUNT_MAKER_EXEC_THRESHOLD (float, default 0.97) — combined bid threshold for execution
"""
...
STATE_F = WORK_DIR / ".poly_sol15m_discount_state.json"
LOG_F = Path("/tmp/polymarket_sol15m_discount.log")
...
WINDOW_SEC = _int("SOL15M_DISCOUNT_WINDOW_SEC", 900)
CHECK_SEC = _int("SOL15M_DISCOUNT_SCAN_SEC", 5)
ACTIVE_LAST_SEC = _int("SOL15M_DISCOUNT_ACTIVE_LAST_SEC", 180)
MIN_LEFT_SEC = _int("SOL15M_DISCOUNT_MIN_LEFT_SEC", 5)
COMBINED_THRESHOLD = _float("SOL15M_DISCOUNT_COMBINED_THRESHOLD", 0.995)
SHADOW_THRESHOLD = _float("SOL15M_DISCOUNT_SHADOW_THRESHOLD", 1.020)
MAX_SPREAD_PER_LEG = _float("SOL15M_DISCOUNT_MAX_SPREAD", 0.02)
MAX_LEG_PRICE = _float("SOL15M_DISCOUNT_MAX_LEG_PRICE", 0.75)
MIN_LIQUIDITY = _float("SOL15M_DISCOUNT_MIN_LIQUIDITY", 1000.0)
PAIR_BUDGET = _float("SOL15M_DISCOUNT_PAIR_BUDGET", 5.0)
MIN_TOP_SIZE = _float("SOL15M_DISCOUNT_MIN_TOP_SIZE", 20.0)
STRONG_EDGE_MIN = _float("SOL15M_DISCOUNT_STRONG_EDGE_MIN", 0.04)
STRONG_SPREAD_MAX = _float("SOL15M_DISCOUNT_STRONG_SPREAD_MAX", 0.01)
DRY_RUN = _bool("SOL15M_DISCOUNT_DRY_RUN", True)
NOTIFY_START = _bool("SOL15M_DISCOUNT_NOTIFY_START", True)

# ── Maker execution config ──
MAKER_EXEC = _bool("SOL15M_DISCOUNT_MAKER_EXEC", False)
MAKER_EXEC_THRESHOLD = _float("SOL15M_DISCOUNT_MAKER_EXEC_THRESHOLD", 0.97)
FILL_TIMEOUT_SEC = _int("SOL15M_DISCOUNT_FILL_TIMEOUT_SEC", 25)
FILL_POLL_SEC = _int("SOL15M_DISCOUNT_FILL_POLL_SEC", 4)

# ── Maker window-open entry config (terminal-bot style) ──
MAKER_ENTRY_WINDOW_SEC = _int("SOL15M_DISCOUNT_MAKER_ENTRY_WINDOW_SEC", 45)
MAKER_ENTRY_DELAY_SEC = _int("SOL15M_DISCOUNT_MAKER_ENTRY_DELAY_SEC", 10)
MAKER_MAX_RETRIES = _int("SOL15M_DISCOUNT_MAKER_MAX_RETRIES", 3)
MAKER_MIN_PRICE = _float("SOL15M_DISCOUNT_MAKER_MIN_PRICE", 0.42)
MAKER_MAX_PRICE = _float("SOL15M_DISCOUNT_MAKER_MAX_PRICE", 0.58)

SERIES_SLUG = "sol-up-or-down-15m"

# ── Scalp strategy config ──
SCALP_ENABLED = _bool("SOL15M_SCALP_ENABLED", False)
SCALP_SIZE = _float("SOL15M_SCALP_SIZE", 5.0)
SCALP_ENTRY_MAX = _float("SOL15M_SCALP_ENTRY_MAX", 0.92)
SCALP_TARGET = _float("SOL15M_SCALP_TARGET", 0.97)
SCALP_STOP = _float("SOL15M_SCALP_STOP", 0.88)
SCALP_SCAN_SEC = _int("SOL15M_SCALP_SCAN_SEC", 120)
...
def get_current_slug() -> tuple[str, int, int]:
    ...
    return f"sol-updown-15m-{window_ts}", window_ts, secs_left
...
def format_alert(opp: dict) -> str:
    ...
    return (
        f"{icon} [SOL-DISCOUNT] DRY-RUN {level}\n"
...
def execute_maker_pair(market: dict, yes_bid: float, no_bid: float, combined_bid: float, secs_left: int):
    ...
    log(
        f"[SOL-MAKER] EXECUTING | YES bid={yes_bid:.4f} NO bid={no_bid:.4f} "
    ...
    if not yes_result["success"] or not no_result["success"]:
        log(f"[SOL-MAKER] order placement failed — YES:{yes_result['success']} NO:{no_result['success']}")
    ...
    entry_msg = (
        f"{sim_prefix}📍🌮 [MAKER] [SOL] entering | "
    ...
    if sim_mode:
        ...
        if roll < 0.55:
            ...
            sign = "+" if pnl >= 0 else "-"
            msg = f"🧪 [SIM] 🟢🌮 [SIM] [SOL] | P&L: {sign}${abs(pnl):.2f} | YES ${yes_bid:.4f} + NO ${no_bid:.4f} = ${combined_bid:.4f}"
            log(msg)
            tg(msg)
            return "both_filled"
        elif roll < 0.80:
            # One fills
            msg = f"🧪 [SIM] 🟠🌮 [SIM] [SOL] | PARTIAL | YES ${yes_bid:.4f} + NO ${no_bid:.4f} = ${combined_bid:.4f}"
            log(msg)
            tg(msg)
            return "partial"
        else:
            # Neither fills
            msg = f"🧪 [SIM] ⚪🌮 [SIM] [SOL] | TIMEOUT | YES ${yes_bid:.4f} + NO ${no_bid:.4f} = ${combined_bid:.4f}"
            log(msg)
            tg(msg)
            return "no_fill"
    ...
    while (time.time() - start) < FILL_TIMEOUT_SEC:
        if not yes_filled and yes_oid:
            ...
            if s == "filled":
                yes_filled = True
                log(f"[SOL-MAKER] YES FILLED @ {yes_bid:.4f} basis={basis} acquired={acquired:.2f}")
            elif s == "partial":
                yes_filled = True
                log(f"[SOL-MAKER] YES PARTIAL/HAVE SHARES @ {yes_bid:.4f} basis={basis} acquired={acquired:.2f}")
            elif s == "cancelled":
                log(f"[SOL-MAKER] YES order cancelled/expired")
                yes_oid = None
        ...
        if not no_filled and no_oid:
            ...
            if s == "filled":
                no_filled = True
                log(f"[SOL-MAKER] NO FILLED @ {no_bid:.4f} basis={basis} acquired={acquired:.2f}")
            elif s == "partial":
                no_filled = True
                log(f"[SOL-MAKER] NO PARTIAL/HAVE SHARES @ {no_bid:.4f} basis={basis} acquired={acquired:.2f}")
            elif s == "cancelled":
                log(f"[SOL-MAKER] NO order cancelled/expired")
                no_oid = None
        ...
        if yes_filled and no_filled:
            ...
            sign = "+" if pnl >= 0 else "-"
            msg = (
                f"🟢🌮 [MAKER] [SOL] | DONE | P&L: {sign}${abs(pnl):.2f} | "
                f"YES ${yes_bid:.4f} + NO ${no_bid:.4f} = ${combined_bid:.4f}"
            )
            log(msg)
            tg(msg)
        ...
        if not yes_oid and not no_oid:
            log("[SOL-MAKER] both orders gone (cancelled/expired)")
            break
    ...
    if yes_filled and not no_filled:
        ...
        if no_oid:
            maker_cancel_order(no_oid)
        msg = (
            f"🟠🌮 [MAKER] [SOL] | HELD_SIDE | P&L: $0.00 | "
    ...
    if yes_final in ("filled", "partial") and no_final in ("filled", "partial"):
        ...
        log(f"[SOL-MAKER] final reconciliation upgraded TIMEOUT->DONE yes={yes_basis}:{yes_acq:.2f} no={no_basis}:{no_acq:.2f}")
        log(msg)
        tg(msg)
        return "both_filled"
    ...
    if yes_final in ("filled", "partial") and no_final not in ("filled", "partial"):
        ...
        log(f"[SOL-MAKER] final reconciliation yes-only basis={yes_basis} acquired={yes_acq:.2f}")
        log(msg)
        tg(msg)
        return "partial_yes"
    ...
    if no_final in ("filled", "partial") and yes_final not in ("filled", "partial"):
        ...
        log(f"[SOL-MAKER] final reconciliation no-only basis={no_basis} acquired={no_acq:.2f}")
        log(msg)
        tg(msg)
        return "partial_no"

    # Truly no fill — cancel both if still open
    if yes_oid:
        maker_cancel_order(yes_oid)
    if no_oid:
        maker_cancel_order(no_oid)
    msg = (
        f"⚪🌮 [MAKER] [SOL] | TIMEOUT | "
        f"YES ${yes_bid:.4f} + NO ${no_bid:.4f} = ${combined_bid:.4f} — No confirmed fill"
    )
    log(msg)
    tg(msg)
    return "no_fill"
...
def check_maker_exec_opportunity(market: dict, secs_left: int, state: dict) -> bool:
    ...
    log(f"[SOL-MAKER] window-open entry mode | elapsed={elapsed}s")
    ...
    for attempt in range(1, MAKER_MAX_RETRIES + 1):
        ...
        if elapsed_now > MAKER_ENTRY_WINDOW_SEC:
            log(f"[SOL-MAKER] entry window expired on attempt {attempt}")
            break
        ...
        if any(v is None for v in [yes_best_bid, no_best_bid, yes_best_ask, no_best_ask]):
            log(f"[SOL-MAKER] attempt {attempt}: incomplete book data")
            continue
        ...
        if cheap_bid < MAKER_MIN_PRICE or cheap_bid > MAKER_MAX_PRICE:
            log(f"[SOL-MAKER] attempt {attempt}: cheap {cheap_side} bid ${cheap_bid:.2f} out of range [{MAKER_MIN_PRICE}-{MAKER_MAX_PRICE}]")
            continue
        ...
        if exp_bid <= 0 or exp_bid >= 1:
            log(f"[SOL-MAKER] attempt {attempt}: expensive bid ${exp_bid:.2f} out of bounds")
            continue
        ...
        if combined_bid > MAKER_EXEC_THRESHOLD:
            log(f"[SOL-MAKER] attempt {attempt}: combined ${combined_bid:.4f} > threshold {MAKER_EXEC_THRESHOLD}")
            continue

        # Success — execute
        edge = round(1.0 - combined_bid, 4)
        log(f"[SOL-MAKER] attempt {attempt}: YES ${yes_bid:.2f} + NO ${no_bid:.2f} = ${combined_bid:.4f} | edge +${edge:.4f}")
    ...
    log(f"[SOL-MAKER] all {MAKER_MAX_RETRIES} attempts exhausted — no entry this window")
    return False
...
def check_scalp_opportunity(market: dict, secs_left: int, state: dict):
    ...
    for i, tid in enumerate(token_ids[:2]):
        ...
        # Entry notification
        entry_msg = (
            f"{sim_prefix}📍🌮 [SCALP] [SOL] entering {entry_label} @ ${entry_price:.2f} "
        ...
        if sim_mode:
            ...
            if roll < 0.55:
                ...
                result_msg = (
                    f"🧪 [SIM] 🟢🌮 [SIM] [SOL] SCALP WIN | {entry_label} ${entry_price:.2f} → "
                ...
                pct = round((pnl / entry_cost) * 100, 2)
                result_msg = (
                    f"🧪 [SIM] 🔴🌮 [SIM] [SOL] SCALP STOP_LOSS | {entry_label} ${entry_price:.2f} → "
                ...
                pct = round((pnl / entry_cost) * 100, 2)
                result_msg = (
                    f"🧪 [SIM] 🟠🌮 [SIM] [SOL] SCALP TIME_STOP | {entry_label} ${entry_price:.2f} → "
        ...
        while (time.time() - monitor_start) < remaining:
            ...
            if sell_oid:
                ...
                if ss == "filled":
                    ...
                    pct = round((pnl / entry_cost) * 100, 2)
                    msg = (
                        f"🟢🌮 [SCALP] [SOL] WIN | {entry_label} ${entry_price:.2f} → "
            ...
            if (time.time() - monitor_start) % 5 < CHECK_SEC + 0.5:
                ...
                if mid is not None and mid <= SCALP_STOP:
                    ...
                    pct = round((pnl / entry_cost) * 100, 2)
                    msg = (
                        f"🔴🌮 [SCALP] [SOL] STOP_LOSS | {entry_label} ${entry_price:.2f} → "
        ...
        if not exited:
            ...
            msg = (
                f"🟠🌮 [SCALP] [SOL] TIME_STOP | {entry_label} ${entry_price:.2f} → "
...
def main():
    ...
    if NOTIFY_START:
        ...
        tg(
            f"🔔 [SOL-DISCOUNT] {mode} watcher started | threshold<={COMBINED_THRESHOLD:.3f}\n"
    ...
    log(
        f"[SOL-DISCOUNT] STARTING dry={DRY_RUN} maker_exec={MAKER_EXEC} "
    ...
    while True:
        try:
            ...
            log("[SOL-DISCOUNT] stopped by user")
            raise
        except Exception as e:
            log(f"[SOL-DISCOUNT] loop error: {e}")

...
PUBLIC_URL = "https://tacoclaw.v-carte.pro"
...
def index():
    """Root endpoint"""
    return jsonify({"status": "ok", "service": "taco-trader-api", "version": "1.0.1"})
...
def tail_logs():
    """Tail trading logs and parse specific events for the dashboard"""
    log_files = [
        "/tmp/polymarket_btc15m.log",
        "/tmp/polymarket_eth15m.log",
        "/tmp/taco_trader.log"
    ]
    
    # Patterns
    # [BTC-MAKER] ORDER PLACED: UP 12.19 shares @ 0.4100 | Order: 0x...
    p_placed = re.compile(r"\\[(.*?)\\] ORDER PLACED: (UP|DOWN) ([\d.]+) shares @ ([\d.]+) \\| Order: (0x[a-fA-F0-9]+)")
    # [BTC-MAKER] ORDER CANCELLED (FOK fallback): 0x... | open_for=73s
    p_cancel = re.compile(r"\\[(.*?)\\] ORDER CANCELLED \\(FOK fallback\\): (0x[a-fA-F0-9]+) \\| open_for=(\d+)s")
    # FILL VERIFIED: BTC-USD 12.19 shares
    p_fill = re.compile(r"FILL VERIFIED: (.*?) ([\d.]+) shares")
    # SKIPPED: SIGNAL_MAX_ENTRY_PRICE
    p_skip = re.compile(r"SKIPPED: (.*)")

    # Seek to end of existing logs
    handles = {}
    for f in log_files:
        if os.path.exists(f):
            h = open(f, 'r')
    ...
    while True:
        for path, h in handles.items():
            ...
            elif "ORDER CANCELLED (FOK fallback)" in line:
                m = p_cancel.search(line)
                if m:
                    event = {
                        "id": str(time.time()),
                        "engine": m.group(1),
                        ...
                        "open_for_seconds": int(m.group(4)),
                        "reason": "fok_fallback",
                        "message": line
                    }
            elif "FILL VERIFIED" in line:
                m = p_fill.search(line)
                if m:
                    event = {
                        "id": str(time.time()),
...
def api_stats_extended():
    """Extended analytics from journal.db"""
    try:
        conn = get_db()
        cur = conn.cursor()
        ...
        cur.execute("""
            SELECT 
                ...
                COUNT(*) as total
            FROM trades
            WHERE exit_type IN ('redeemed', 'filled', 'resolved', 'closed')
        """)
        w_row = cur.fetchone()
        
        # 2. Best Trade
        cur.execute("""
            SELECT asset, pnl_absolute, pnl_percent 
            FROM trades 
            ORDER BY pnl_absolute DESC LIMIT 1
        """)
        b_row = cur.fetchone()
        
        # 3. Total Volume (USD)
        cur.execute("SELECT SUM(position_size_usd) FROM trades")
        v_row = cur.fetchone()
        
        # 4. Engine Performance
        cur.execute("""
            SELECT engine, SUM(pnl_absolute) as pnl
            FROM trades
            GROUP BY engine
            ORDER BY pnl DESC
        """)
        engines = {row[0]: row[1] for row in cur.fetchall()}
        ...
        return jsonify({
            "wins": w_row[0] or 0,
            "losses": w_row[1] or 0,
            "total_trades": w_row[2] or 0,
            "win_rate": round((w_row[0]/w_row[2])*100, 2) if w_row[2] else 0,
            "best_trade": {"asset": b_row[0], "pnl": b_row[1]} if b_row else None,
            "total_volume_usd": round(v_row[0] or 0, 2),
            "engine_performance": engines
        })
    except Exception as e:
        return jsonify({"error": str(e)}), 500
